  ...
  return (
    <>
      <button className="certificate-highlight" type="button" onClick={() => setSelected({ name: "Guinness World Records certificate", image: guinnessImage })}>
        ...
        <div><p className="section-kicker">Achievement evidence</p><h3>Guinness World Records certificate.</h3><p>The certificate records Modon Properties in association with Petroleum Machinery and Technical Equipment, Abu Dhabi, for the controlled demolition on 27 November 2020.</p><span>View certificate ↗</span></div>
      </button>
      <div className="certificate-library">
        ...
        <div className="certificate-list">{certificates.map((certificate) => <button key={certificate.name} type="button" onClick={() => setSelected(certificate)}><span>PDF</span>{certificate.name}<b>View ↗</b></button>)}</div>
      </div>
