...
export default function ArabicPortfolio() {
  return <main className="arabic-page" dir="rtl" lang="ar">
    ...
    <section className="hero arabic-hero" id="home">
      ...
      <div className="hero-stats" aria-label="ملخص الشركة"><div><strong>1994</strong><span>تاريخ التأسيس</span></div><div><strong>30+</strong><span>سنة من الخبرة</span></div><div><strong>8</strong><span>أنشطة مرخّصة</span></div><div><strong>25+</strong><span>مركبة ومعدة مسجلة</span></div></div>
    </section>
    ...
    <section className="projects-section section-shell" id="projects"><div className="section-index">03 · مشاريع مختارة</div><div className="split-heading"><div><p className="section-kicker">خبرات موثقة</p><h2>عمل واضح.<br /><span>نطاق محدد.</span></h2></div><p>استعرض مشاريع PMTE في الهدم والأعمال الترابية والبحرية في أبوظبي والعين.</p></div><div className="project-grid">{projects.map(([title, location, scope, image, slug, representative], index) => <article className="project-card" key={title}><div className="project-image"><OptimizedImage src={image} alt={representative ? `صورة تمثيلية لبيئة مشروع ${title}` : `${title} — ${scope} في ${location}`} loading="lazy" widths={[480, 768, 1100]} sizes="(max-width: 760px) 100vw, (max-width: 1280px) 50vw, 33vw" /><span>{String(index + 1).padStart(2, "0")}</span>{representative ? <small className="project-image-note">صورة تمثيلية</small> : null}</div><div className="project-body"><p>مشاريع PMTE</p><h3><a href={`/ar/projects/${slug}`}>{title}</a></h3><dl><div><dt>الموقع</dt><dd>{location}</dd></div><div><dt>نطاق العمل</dt><dd>{scope}</dd></div></dl></div></article>)}</div><p className="arabic-gallery-link"><a href="/ar/gallery">استعرض معرض الصور الكامل ←</a></p></section>

    <section className="clients-section" id="clients"><div className="section-shell"><div className="section-index light">04 · خبرات مع العملاء</div><div className="split-heading light-heading"><div><p className="section-kicker">سجل العملاء السابقين</p><h2>موثوقون عبر<br /><span>أبوظبي.</span></h2></div><p>تشمل جهات عملت معها PMTE قطاعات الرعاية الصحية والإعلام والعقارات والاستثمار في أبوظبي.</p></div><div className="client-grid">{clients.map(([name, image]) => <article className="client-card" key={name}><img src={image} alt={`شعار ${name}`} loading="lazy" /></article>)}</div></div></section>

    <section className="record-section" id="achievement"><OptimizedImage className="record-photo" src="/assets/projects/mina-plaza-aerial-before-demolition.jpg" alt="أبراج مينا بلازا في أبوظبي قبل عملية الهدم المنضبط الموثقة" loading="lazy" widths={[768, 1280, 1920]} quality={88} /><div className="record-shade" aria-hidden="true" /><div className="record-content"><p className="section-kicker">إنجاز رئيسي · 27 نوفمبر 2020</p><p className="record-number">165.032 <span>متراً</span></p><h2>بالاشتراك في إنجاز هدم منضبط مسجل في غينيس للأرقام القياسية.</h2><p>ارتبطت PMTE مع شركة مدن العقارية في عملية الهدم المنضبط لمبنى بارتفاع 165.032 متراً في أبوظبي بتاريخ 27 نوفمبر 2020.</p><a className="record-evidence-link" href="#credentials">عرض شهادة الدعم <span aria-hidden="true">↓</span></a></div></section>

    <section className="fleet-section section-shell" id="fleet"><div className="section-index">05 · الأسطول والمعدات</div><div className="fleet-top"><div className="section-title"><p className="section-kicker">تجهيز مخصص للمشروع</p><h2>معدات مناسبة<br /><span>لطبيعة العمل.</span></h2></div><div className="fleet-copy"><p>يضم أسطول PMTE المسجل أكثر من 25 مركبة ومعدة لأعمال الهدم والأعمال الترابية والرفع والنقل.</p><div className="verified-models"><article><span>طراز موثق</span><strong>LiuGong 856H</strong><p>لودر بعجلات</p></article><article><span>طراز موثق</span><strong>Caterpillar 340 D2 L</strong><p>حفارة</p></article></div></div></div><div className="equipment-strip"><OptimizedImage src="/assets/equipment/excavator-demolition.jpg" alt="حفارة هدم ثقيلة تعمل في موقع بالإمارات" loading="lazy" widths={[420, 720, 960]} sizes="(max-width: 760px) 100vw, 25vw" /><OptimizedImage src="/assets/equipment/mobile-crane.jpg" alt="رافعة متحركة لعمليات الرفع في أبوظبي" loading="lazy" widths={[420, 720, 960]} sizes="(max-width: 760px) 100vw, 25vw" /><OptimizedImage src="/assets/equipment/material-handling.jpg" alt="معدات مناولة المواد في مواقع العمل" loading="lazy" widths={[420, 720, 960]} sizes="(max-width: 760px) 100vw, 25vw" /><OptimizedImage src="/assets/equipment/lifting-operations.jpg" alt="عملية رفع ثقيلة بمعدات PMTE" loading="lazy" widths={[420, 720, 960]} sizes="(max-width: 760px) 100vw, 25vw" /></div><ul className="equipment-list" aria-label="فئات المعدات"><li><span>01</span>حفارات</li><li><span>02</span>لودرات بعجلات</li><li><span>03</span>رافعات متحركة</li><li><span>04</span>جريدرات</li><li><span>05</span>شاحنات قلاب</li><li><span>06</span>مقطورات</li><li><span>07</span>معدات قطع</li><li><span>08</span>ملحقات الهدم</li></ul></section>
