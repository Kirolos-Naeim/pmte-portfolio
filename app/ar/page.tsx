...
const capabilities = [
  { title: "الهدم وإخراج المنشآت من الخدمة", copy: "هدم إنشائي وتفكيك منضبط وإزالة مبانٍ ودعم أعمال إخراج المنشآت من الخدمة.", Icon: Hammer },
  { title: "الحفريات والأعمال الترابية", copy: "حفر وقطع وردم وتسوية وتجهيز المواقع وأعمال الأساسات.", Icon: Tractor },
  { title: "الأعمال البحرية", copy: "دعم الموانئ والسواحل وإزالة الكتل الخرسانية تحت الماء وترتيب الصخور.", Icon: Waves },
  ...
  { title: "توريد المعدات والدعم الفني", copy: "توريد معدات وقطع غيار وحواجز وبوابات إلكترونية وحلول دعم فني ذات صلة.", Icon: PackageSearch },
];

const projects = [
  ["مركز أدنيك · المراحل 1–4", "أبوظبي", "هدم وإزالة متعددة المراحل", "/assets/projects/gallery-015-p15-01.jpg"],
  ["هدم مينا بلازا", "ميناء زايد، أبوظبي", "دعم هدم الأبراج بصورة منضبطة", "/assets/projects/mina-plaza-aerial-before-demolition.jpg"],
  ["كمارت أبوظبي", "أبوظبي", "هدم مبانٍ وإخلاء الموقع", "/assets/projects/gallery-025-p21-01.jpg"],
  ["مباني ميناء زايد", "ميناء زايد، أبوظبي", "إزالة مبانٍ وتسوية الموقع", "/assets/projects/gallery-035-p28-01.jpg"],
  ["ميناء زايد البحري", "أبوظبي", "إزالة كتل خرسانية تحت الماء", "/assets/projects/gallery-049-p39-01.jpg"],
  ["حديقة الحيوانات بالعين", "العين", "قطع وردم وهدم وحفريات", "/assets/projects/gallery-069-p54-01.jpg"],
];
  ...
  return <main className="arabic-page" dir="rtl" lang="ar">
    ...
    <section className="hero arabic-hero" id="home">
      ...
      <div className="hero-content">
        ...
        <h1>هدم منضبط.<br /><span>تنفيذ موثوق.</span></h1>
        <p className="hero-copy">حلول الهدم والأعمال الترابية والأعمال البحرية والدعم الصناعي.</p>
    ...
    <section className="capabilities-section" id="services"><div className="section-shell"><div className="section-index light">02 · الخدمات الأساسية</div><div className="split-heading light-heading"><div><p className="section-kicker">دعم متكامل للمواقع</p><h2>شريك ميداني<br /><span>واحد ومسؤول.</span></h2></div><p>أنشطة مرخّصة وخبرة تنفيذ موثقة للبيئات الإنشائية وبيئات الهدم والواجهات البحرية في دولة الإمارات.</p></div><div className="capability-grid">{capabilities.map(({ title, copy, Icon }, index) => <article className="capability-card" key={title}><span className="capability-code">{String(index + 1).padStart(2, "0")}</span><Icon className="capability-icon" aria-hidden="true" /><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>

    <section className="projects-section section-shell" id="projects"><div className="section-index">03 · مشاريع مختارة</div><div className="split-heading"><div><p className="section-kicker">خبرات موثقة</p><h2>عمل واضح.<br /><span>نطاق محدد.</span></h2></div><p>تعكس أسماء المشاريع ونطاقاتها ملف الشركة المقدم. لم تتم إضافة قيم عقود أو نسب إنجاز أو مؤشرات أداء غير موثقة.</p></div><div className="project-grid">{projects.map(([title, location, scope, image], index) => <article className="project-card" key={title}><div className="project-image"><img src={image} alt={`صورة مشروع ${title}`} loading="lazy" /><span>{String(index + 1).padStart(2, "0")}</span></div><div className="project-body"><p>مشاريع PMTE</p><h3>{title}</h3><dl><div><dt>الموقع</dt><dd>{location}</dd></div><div><dt>نطاق العمل</dt><dd>{scope}</dd></div></dl></div></article>)}</div><p className="arabic-gallery-link"><a href="/ar/gallery">استعرض معرض الصور الكامل ←</a></p></section>
